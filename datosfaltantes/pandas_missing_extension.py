...
# Assuming you have a DataFrame riskfactors_df loaded
...
def missing_variable_plot(df):
    ...
    missing_summary = missing_summary.sort_values('n_missing')

    # Plot
    plot_range = range(1, len(missing_summary) + 1)
    plt.hlines(y=plot_range, xmin=0, xmax=missing_summary['n_missing'], color='black')
    plt.plot(missing_summary['n_missing'], plot_range, 'o', color='black')
    plt.yticks(plot_range, missing_summary['variable'])
    plt.grid(axis='y')
